/**
 * Register Yield Optimizer Agent On-Chain
 * 
 * This script registers the Yield Optimizer agent with its own dedicated wallet
...
async function main() {
    console.log("\n" + "=".repeat(60));
    console.log("💰 REGISTERING YIELD OPTIMIZER AGENT ON-CHAIN");
    ...
    // Create or use dedicated Yield Optimizer wallet
    let yieldOptimizerWallet: Wallet;
    if (process.env.YIELD_OPTIMIZER_PRIVATE_KEY) {
        yieldOptimizerWallet = new Wallet(process.env.YIELD_OPTIMIZER_PRIVATE_KEY, provider);
        console.log(`💰 Using existing Yield Optimizer wallet: ${yieldOptimizerWallet.address}`);
    } else {
        // Generate new wallet for Yield Optimizer agent
        yieldOptimizerWallet = Wallet.createRandom().connect(provider);
        console.log(`\n⚠️  Generated NEW Yield Optimizer wallet!`);
        console.log(`   Address: ${yieldOptimizerWallet.address}`);
        console.log(`   Private Key: ${yieldOptimizerWallet.privateKey}`);
        console.log(`\n   📝 Add this to your .env file:`);
        console.log(`   YIELD_OPTIMIZER_PRIVATE_KEY=${yieldOptimizerWallet.privateKey}`);
    }

    // Connect to registry
    const registry = new ethers.Contract(registryAddress, REGISTRY_ABI, deployer);

    // Check if Yield Optimizer agent already exists
    console.log(`\n🔍 Checking for existing yield_optimization agents...`);
    try {
        const existingIds = await registry.getAgentIdsByCapability('yield_optimization');
    ...
    // Define the Yield Optimizer agent
    const agent = {
        name: "Yield-Optimizer",
        capability: "yield_optimization",
        wallet: yieldOptimizerWallet.address,
        owner: deployer.address, // Owner is the deployer (receives payments)
        price: 40000, // $0.04 USDC (6 decimals)
        endpoint: "groq:llama-3.3-70b-versatile"
    };

    console.log(`\n📋 Registering Yield Optimizer Agent:`);
    ...
    console.log(`\n🎉 Yield Optimizer Agent registered with Token ID: ${tokenId}`);
